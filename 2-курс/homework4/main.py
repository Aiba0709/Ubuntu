...
class Berserk(Heroes):
    def __init__(self, name, health, damage):
        super(Berserk, self).__init__(name, health, damage, "SMART")
...
class Thor(Heroes):
    def __init__(self, name, health, damage):
        super(Thor, self).__init__(name, health, damage, "PLUG")
    def apply_super_power(self, boss, heroes):
...
def start_game():
    ...
    berserk = Berserk("Aiba", 250, 8 )
    thor = Thor("CHika", 100, 10)
    witcher = Witcher("Ali", 200, 0)
    heroes = [warrior, magic, medic, berserk, thor, witcher]
